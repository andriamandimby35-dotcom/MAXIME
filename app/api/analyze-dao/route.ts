    ...
    const aiResponse = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Erreur constatée à éviter absolument sur ce genre de document multi-pages (CCAP, calendrier cultural, code de conduite, règlement de chantier, convention entre parties...) : template_text ne doit JAMAIS s'arrêter après la première page ou le premier article sous prétexte que le titre du document a déjà été repéré. Le CCAP en particulier s'étend très souvent sur PLUSIEURS pages consécutives (5, 10 pages ou davantage), avec de nombreux articles numérotés (Article 1, Article 2, Article 3...) : lis et reproduis dans template_text le texte COMPLET de TOUTES ces pages, dans leur ordre, jusqu'à la dernière page où ce document se termine réellement (juste avant qu'un autre titre du DAO ne prenne le relais) — jamais un extrait tronqué d'une seule page. Sépare toujours chaque article par une ligne vide, exactement comme pour des paragraphes numérotés, et transforme chaque blanc à compléter en {{cle_du_champ}} comme pour tout autre modèle.",
          ...
          "Erreur fréquente à éviter dans un modèle de garantie bancaire, de caution personnelle ou tout autre engagement d'un tiers (banque, assurance...) : un numéro de garantie, un numéro de caution ou un numéro de marché n'existe pas encore au moment de la soumission — ils ne sont attribués qu'après coup, par la banque ou par l'autorité contractante lors de la signature. Ne recopie donc JAMAIS le numéro de l'avis d'appel d'offres, la référence du DAO ou tout autre numéro déjà connu dans un champ désignant un numéro de garantie, un numéro de caution ou un numéro de marché : ce sont des identifiants distincts, produits à des moments différents, même quand leurs libellés se ressemblent. Laisse ce champ sans valeur préremplie (fields sans entrée correspondante dans prefilled_values) sauf si le DAO donne réellement, explicitement, CETTE valeur précise pour CE champ précis.",
          ...
          "IMPORTANT pour éviter une réponse tronquée (trop longue et donc invalide) : template_fill_positions ne doit PLUS être rempli pour un champ déjà couvert par {{cle_du_champ}} dans template_text — dupliquer la même information aux deux endroits (texte réécrit ET position sur la page) alourdit énormément la réponse pour rien, puisque l'application n'utilise déjà plus ces positions dès que template_text existe. Ne renseigne template_fill_positions QUE pour un champ d'une pièce SANS aucun template_text (document purement graphique, plan, ou cas rare où aucun texte n'a pu être établi) : dans ce cas seulement, une position (page, field_key, x_percent, y_percent, width_percent) reste nécessaire pour CHAQUE champ concerné, estimée à partir du blanc/tiret/case visible sur la page.",
          "Cette même règle s'applique aux cellules de tableau : une cellule déjà couverte par {{cle_du_champ}} dans template_tables n'a besoin d'AUCUNE position dans template_fill_positions — le tableau structuré (template_tables) suffit entièrement à l'application. Ne renseigne une position de cellule que pour une pièce sans template_text ni template_tables exploitable pour cette cellule précise.",
        ].join(" "),
        input: [{
          role: "user",
          content: [
