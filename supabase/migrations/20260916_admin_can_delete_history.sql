-- Donne à l'administrateur (et à lui seul) la possibilité de supprimer les
-- photos, rapports journaliers, mouvements de stock, achats, présences et
-- membres d'équipe d'un chantier, quel que soit le jour où ils ont été
-- ajoutés — et corrige au passage le planning DAO qui ne remontait pas
-- toujours pour les comptes conducteur/chef.
--
-- À exécuter une seule fois dans Supabase > SQL Editor.

-- 1) Le "verrou du jour" empêchait TOUT LE MONDE (même l'administrateur) de
-- modifier ou supprimer une saisie plus ancienne qu'aujourd'hui. On ajoute
-- une exception pour l'administrateur de l'organisation ; pour tous les
-- autres, rien ne change (toujours verrouillé après le jour de saisie).
...
-- 2) Le planning du DAO doit être visible par toute l'équipe affectée au
-- chantier (administrateur, conducteur, chef) — pas seulement en lecture
-- pour l'administrateur. On réaffirme cette règle explicitement.
...
-- 3) Suppression réservée à l'administrateur, pour chaque type de saisie
-- terrain. On remplace toute règle de suppression existante (qui autorisait
-- parfois aussi l'auteur du jour) par une règle unique : administrateur
-- uniquement.
...
-- Les photos et l'équipe déclarée utilisaient une règle "tout en un" (lecture,
-- ajout, modification ET suppression confondus) : on la sépare pour garder
-- exactement le même comportement d'ajout/modification, mais réserver la
-- suppression à l'administrateur.
...
-- 4) Pour annuler proprement l'effet sur le stock quand on supprime un
-- mouvement ou un achat déjà validé, on retrouve précisément d'où vient
-- chaque mouvement de stock : le rapport journalier qui l'a généré, ou
-- l'achat qui l'a généré.
...
-- La consommation déclarée dans un rapport journalier note désormais le
-- rapport d'origine sur son mouvement de stock (pour pouvoir tout annuler
-- proprement si le rapport est supprimé).
...
-- 5) Suppression d'un mouvement de stock (l'"historique") : annule son effet
-- sur le stock du matériau concerné, retire la consommation liée dans le
-- rapport journalier si elle existe, puis retire le mouvement.
...
-- 6) Suppression d'une demande/achat de matériau : si l'achat avait déjà été
-- validé (statut "paid"), on retire du stock la quantité qu'il avait
-- ajoutée et on supprime le mouvement de stock qu'il avait créé, avant de
-- retirer la demande elle-même.
...
-- 7) Suppression d'un rapport journalier : annule toute la consommation de
-- stock qu'il avait déclarée, supprime les mouvements de stock liés,
-- détache ses photos (elles réapparaissent dans "Photos non classées" au
-- lieu d'être perdues), puis retire le rapport.
