-- Consommation de stock depuis le rapport journalier.
-- Chaque déduction est enregistrée avec son rapport et son mouvement de stock.

create table if not exists public.project_report_material_usages (
  ...
  material_id uuid not null references public.project_materials(id) on delete restrict,
...
create index if not exists project_report_material_usages_project_idx
  on public.project_report_material_usages(project_id, report_id);

alter table public.project_report_material_usages enable row level security;

drop policy if exists project_report_material_usages_read on public.project_report_material_usages;
create policy project_report_material_usages_read on public.project_report_material_usages
  for select to authenticated using (public.can_access_project(project_id));

-- Le chef de chantier peut ajouter les membres qu'il va pointer dans son rapport.
...
create policy project_staff_members_manage_attendance on public.project_staff_members
  for all to authenticated
  using (public.can_manage_project_attendance(project_id))
...
grant select on public.project_report_material_usages to authenticated;
grant execute on function public.record_project_daily_report_consumption(uuid, date, text, integer, text, text, text, jsonb) to authenticated;
