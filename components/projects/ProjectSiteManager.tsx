      ...
      <main className="projectSiteGrid">
        {(isAdmin || accessRole === "works_manager" || isSiteManager) && <section className={`projectSiteCard projectAccessCard${isAdmin ? " mobAdminOrder7" : ""}`}>
          ...
          <div className="projectStaffPanelForm" style={{ marginTop: "18px", paddingTop: "16px", borderTop: "1px solid var(--line)" }}>
            ...
            {isAdmin && <article><h3 style={{cursor:"pointer"}} onClick={() => setExpandedRoster((current) => ({ ...current, conductors: !current.conductors }))}>{expandedRoster.conductors ? "▾" : "▸"} Conducteurs</h3><p>{activeConductors.length} actif(s) · {pendingConductors.length} en attente</p>{expandedRoster.conductors && <>{activeConductors.map((item) => <div className="projectTeamMember" key={item.id} data-revealable onClick={() => isAdmin && !projectFinished && setRevealedKey((current) => current === `assignment-${item.id}` ? null : `assignment-${item.id}`)}><strong style={isAdmin ? {cursor:"pointer",textDecoration:"underline"} : undefined} onClick={(event) => { event.stopPropagation(); isAdmin && setViewingAssignment(item); }}>Compte {conductorLabel(item.id, "conducteur").toLowerCase()} · {item.displayName || item.email || item.user_id.slice(0, 8)}</strong>{phoneLink(item.phone_number)}<div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }}><span className="active">Actif</span>{isAdmin && !projectFinished && revealedKey === `assignment-${item.id}` && <button type="button" className="dangerButton" disabled={busy} onClick={(event) => { event.stopPropagation(); revokeAccess({ assignmentId: item.id }, "cet accès conducteur"); }}>Retirer</button>}</div></div>)}{pendingConductors.map((item) => <div className="projectTeamMember" key={item.id} data-revealable onClick={() => isAdmin && !projectFinished && setRevealedKey((current) => current === `invite-${item.id}` ? null : `invite-${item.id}`)}><strong>{item.email}</strong><div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }}><span className="pending">En attente</span>{isAdmin && !projectFinished && revealedKey === `invite-${item.id}` && <><button type="button" className="ghostButton" disabled={busy} onClick={(event) => { event.stopPropagation(); openInvitationEdit(item); }}>Modifier</button><button type="button" className="dangerButton" disabled={busy} onClick={(event) => { event.stopPropagation(); revokeAccess({ invitationId: item.id }, "cette demande d’accès"); }}>Supprimer</button></>}</div></div>)}{activeConductors.length + pendingConductors.length === 0 && <small>Aucun conducteur créé.</small>}</>}</article>}
            <article><h3 style={{cursor:"pointer"}} onClick={() => setExpandedRoster((current) => ({ ...current, siteManagers: !current.siteManagers }))}>{expandedRoster.siteManagers ? "▾" : "▸"} Chefs de chantier</h3><p>{activeSiteManagers.length} actif(s) · {pendingSiteManagers.length} en attente</p>{expandedRoster.siteManagers && <>{activeSiteManagers.map((item) => <div className="projectTeamMember" key={item.id} data-revealable onClick={() => isAdmin && !projectFinished && setRevealedKey((current) => current === `assignment-${item.id}` ? null : `assignment-${item.id}`)}><strong style={isAdmin ? {cursor:"pointer",textDecoration:"underline"} : undefined} onClick={(event) => { event.stopPropagation(); isAdmin && setViewingAssignment(item); }}>Compte chef de chantier · {item.displayName || item.email || item.user_id.slice(0, 8)}</strong>{phoneLink(item.phone_number)}<div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }}><span className="active">Actif</span>{isAdmin && !projectFinished && revealedKey === `assignment-${item.id}` && <button type="button" className="dangerButton" disabled={busy} onClick={(event) => { event.stopPropagation(); revokeAccess({ assignmentId: item.id }, "cet accès chef de chantier"); }}>Retirer</button>}</div></div>)}{pendingSiteManagers.map((item) => <div className="projectTeamMember" key={item.id} data-revealable onClick={() => isAdmin && !projectFinished && setRevealedKey((current) => current === `invite-${item.id}` ? null : `invite-${item.id}`)}><strong>{item.email}</strong><div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }}><span className="pending">En attente</span>{isAdmin && !projectFinished && revealedKey === `invite-${item.id}` && <><button type="button" className="ghostButton" disabled={busy} onClick={(event) => { event.stopPropagation(); openInvitationEdit(item); }}>Modifier</button><button type="button" className="dangerButton" disabled={busy} onClick={(event) => { event.stopPropagation(); revokeAccess({ invitationId: item.id }, "cette demande d’accès"); }}>Supprimer</button></>}</div></div>)}{activeSiteManagers.length + pendingSiteManagers.length === 0 && <small>Le conducteur créera les chefs de chantier qui lui sont rattachés.</small>}</>}</article>
            ...
            <article><h3>Équipe déclarée</h3><p>{projectStaff.length} personne(s) active(s)</p>{projectStaff.length ? projectStaff.map((member) => <div className="projectTeamMember" key={member.id} data-revealable onClick={() => isAdmin && !member.linked_assignment_id && setRevealedKey((current) => current === `staff-${member.id}` ? null : `staff-${member.id}`)}><strong>{member.full_name}</strong>{phoneLink(member.mvola_number)}<div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }} onClick={(event) => event.stopPropagation()}>{(() => {
...
<div className="modalBackdrop" onClick={() => setViewingReportDetail(null)}><div className="modal" onClick={(event) => event.stopPropagation()} style={{ width: "min(560px,100%)", display: "flex", flexDirection: "column", maxHeight: "85vh" }}>
          ...
          <div className="buttonRow" style={{ flex: "0 0 auto", justifyContent: "flex-end", marginBottom: 0 }}>
            {isAdmin && <button type="button" className="dangerButton" disabled={busy} onClick={() => void adminDeleteReport(viewingReportDetail)}>Supprimer ce rapport</button>}
            <button type="button" className="ghostButton" onClick={() => setViewingReportDetail(null)}>Fermer</button>
          </div>
        </div></div>, document.body)}
...
<div className="modalBackdrop" onClick={() => { setViewingPhotoUrl(null); setViewingPhotoRecord(null); }}><div className="modal" onClick={(event) => event.stopPropagation()} style={{ width: "min(640px,100%)" }}>
          ...
          {isAdmin && viewingPhotoRecord && photos.some((item) => item.id === viewingPhotoRecord.id) && <button type="button" className="dangerButton mt-3" disabled={busy} onClick={() => void adminDeletePhoto(viewingPhotoRecord)}>Supprimer cette photo</button>}
...
<div className="modalBackdrop" onClick={() => setViewingAchats(false)}><div className="modal" onClick={(event) => event.stopPropagation()} style={{ width: "min(480px,100%)", display: "flex", flexDirection: "column", maxHeight: "85vh" }}>
            ...
            <div className="projectNoteList projectNoteListScroll" style={{ flex: "1 1 auto", minHeight: 0 }}>
              {paidOrdersHistory.length ? paidOrdersHistory.map((order) => <div className="projectTeamMember" key={order.id}><strong>{order.material_name}</strong><div className="buttonRow" style={{ justifyContent: "flex-end", marginBottom: 0 }}><span>Demande : {order.submitted_at ? dateTime.format(new Date(order.submitted_at)) : "—"} · Validation : {order.approved_at ? dateTime.format(new Date(order.approved_at)) : "—"} · Achat : {order.paid_at ? dateTime.format(new Date(order.paid_at)) : "—"}{order.purchase_photo_path && <button type="button" className="ghostButton" style={{ marginLeft: "8px" }} onClick={() => void openPhoto({ id: order.id, project_id: selectedId || "", storage_path: order.purchase_photo_path!, caption: order.purchase_photo_caption || null, photo_type: "delivery", captured_at: order.paid_at || order.approved_at || new Date().toISOString(), created_at: order.paid_at || order.approved_at || new Date().toISOString() } as SitePhoto)}>Photo</button>}</span>{isAdmin && <button type="button" className="dangerButton" disabled={busy} onClick={() => void adminDeleteMaterialOrder(order)}>Supprimer</button>}</div></div>) : <p className="projectEmptyText">Aucun achat payé pour l’instant.</p>}
            </div>
